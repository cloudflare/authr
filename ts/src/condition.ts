...
const operators: Map<OperatorSign, IOperatorFunc> = new Map([
  [OperatorSign.EQUALS, (left: any, right: any): boolean => left == right],
  [OperatorSign.NOT_EQUALS, (left: any, right: any): boolean => left != right],
  [
    OperatorSign.LIKE,
    (left: any, right: any): boolean => {
      let pleft = "^";
      ...
      return RegExp(`${pleft}${right.replace("*", "")}${pright}`, "i").test(
        left
      );
    },
  ],
  ...
  [
    ...
    (left: any, right: any): boolean => {
      ...
      return (
        intersectionWith(left, right, (a: any, b: any) => a == b).length > 0
      );
    },
  ],
  [
